...
**Guide coverage:** Priority 2, Days 4–6
...
- Connect use-case tiering to the later assessment, testing, and monitoring work planned for Priority 2 without duplicating it.
...
Tiering also is not permanent. A use case's tier can change when its purpose, population, autonomy, or data changes — the same discipline Priority 1, Module 9 applies to material change in an agent's lifecycle, now applied one level up, at the point a use case is first classified rather than after it already holds privileged access.
...
A tier assigned at intake can become wrong without any code change, for the same reasons Priority 1, Module 9 treats "no code changed" as insufficient to prove a deployment is unchanged:
...
1. Why is use-case classification a different question from Priority 1, Module 6's action classification?
...
Complete the [Priority 2, Module 2 Participant Workbook](./exercises/03_Module_02_Participant_Workbook.md) before reading the [Model Answer and Review Guide](./exercises/04_Module_02_Model_Answer_and_Review_Guide.md).
