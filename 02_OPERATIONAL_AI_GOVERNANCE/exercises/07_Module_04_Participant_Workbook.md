...
In your own words, state the distinct question each of verification, validation, testing, and evaluation answers. For each, give one example of a result that could look like a "pass" on the other three terms while still failing this one.

## Part 2 — TEVV plan

Using the minimum specification in Section 3 of the lesson, draft a TEVV plan for the benefits eligibility recommendation use case. Include at least:
...
For each test in your Part 2 plan, write the pass/fail criterion as you would record it before running the test — specific enough that a result could not be reinterpreted afterward to fit. State who would need to approve each criterion before testing begins.
...
- one place where a vendor's benchmark or demonstration might be mistaken for adequate TEVV, and how your plan avoids that;
- one criterion in your Part 3 that you set deliberately strict, and why you would resist softening it after a disappointing result.
